...
export const getCo = createAsyncThunk(
  "Co/getCo",
  async (args, thunkApi) => {
    try {
...
const CoSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(getCo.pending, (state) => {
        state.isLoading = true;
        state.error = null;
      })
      .addCase(getCo.fulfilled, (state, { payload }) => {
        console.log(payload);
      ...
      .addCase(getCo.rejected, (state, { payload }) => {
        state.isLoading = false;
...
export default CoSlice.reducer;
